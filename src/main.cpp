...
#if defined(NDEBUG)
# error "Batman cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6aca240fd5b16e5694abc0fa552fb2d3bd0f6122d9025fc3f2b202f39a8e5c0a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Batman: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Batman Signed Message:\n";
...
{
    // Batman: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Batman: 1 day
static const int64 nTargetSpacing = 1 * 60; // Batman: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Batman: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Batman: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x77b89aa79a660fc10008dbfe3f544dd6f095fc1b6b9acb7adf5227a00d922d1d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b38ffe51aeead736f13bf94bc48617358f83e20713d3b9221afd734732e6da61067e927d1856dca8e70a676aa7f954efbe2c2fb86d3c275b01d8eb31e278ab19") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1532024146;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084799691;

        if (fTestNet)
        {
            block.nTime    = 1532024138;
            block.nNonce   = 385438327;
        }
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0x6f76c0a80f5fa0e1d75805138fd96068d76c2d6e194561061afbea02e66ac3ab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4e, 0xcc, 0xd3, 0xba }; // Batman: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BatmanMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BatmanMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BatmanMiner(CWallet *pwallet)
{
    printf("BatmanMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("batman-miner");
    ...
    try { loop {
        ...
        printf("Running BatmanMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BatmanMiner, pwallet));
}
